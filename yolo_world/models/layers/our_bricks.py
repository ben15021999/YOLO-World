...
class SoftmaxSigmoidAttnBlock(BaseModule):
    """Improved Sigmoid Attention Block with softmax over text and residual blending."""
    ...
    def __init__(self,
        ...
        assert (out_channels % num_heads == 0 and embed_channels % num_heads == 0), \
            'out_channels and embed_channels must be divisible by num_heads.'
    ...
    def forward(self, x: Tensor, guide: Tensor) -> Tensor:
        ...
        guide = guide.reshape(B, -1, self.num_heads,
                              self.head_channels)  # [B, N, H, C]
        embed = self.embed_conv(x) if self.embed_conv is not None else x
        embed = embed.reshape(B, self.num_heads, self.head_channels, H, W)

        # Compute attention weights (softmax over guide tokens)
        # attn_weight = torch.einsum(
        #     'bmchw,bnhc->bmnhw', embed, guide)  # [B, M, N, H, W]
        if self.use_einsum:
        ...
        attn_weight = torch.softmax(attn_weight, dim=2)  # softmax over N
        attn_weight = attn_weight.sum(dim=2)  # aggregate over guide tokens
        attn_weight = attn_weight + self.bias[None, :, None, None]
        attn_weight = attn_weight.sigmoid(
        ) * self.scale  # [B, num_heads, H, W]

        # Project features
        x_proj = self.project_conv(x)  # [B, out_channels, H, W]
        x_proj = x_proj.reshape(B, self.num_heads, -1, H, W)

        # Apply attention
        x_attn = x_proj * attn_weight.unsqueeze(2)  # apply attention per head
        x_out = x_attn.reshape(B, -1, H, W)

        # Residual blending with original projection
        x_proj = x_proj.reshape(B, -1, H, W)
        x = 0.5 * x_proj + 0.5 * x_out  # simple residual blend
        return x
    

@MODELS.register_module()
class SoftmaxSigmoidCSPLayerWithTwoConv(CSPLayerWithTwoConv):
    ...
    def __init__(
        ...
        self.attn_block = SoftmaxSigmoidAttnBlock(self.mid_channels,
                                                  self.mid_channels,
