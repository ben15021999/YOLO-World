...
class YOLOv5CustomMixedGroundingDataset(BatchShapePolicyDataset, CocoDataset):
            ...
            with open(local_path, 'r', encoding='utf-8') as f:
                for line in f:
                    ...
                    raw_img_info = {
                        ...
                        'width': record['width'],
                        'img_id': record.get('id', hash(record['filename'])),
                        'caption': caption,
                    }
                    # Normalize all boxes to COCO-like dicts
                    raw_ann_info = []
                    for r in regions:
                        bboxes = r['bbox']
                        # Support multiple bboxes (list of boxes)
                        if isinstance(bboxes[0], list):  # multiple boxes
                            for box in bboxes:
                                x1, y1, x2, y2 = box
                                w, h = x2 - x1, y2 - y1
                                raw_ann_info.append({
                                    ...
                                    'area': w * h,
                                    'iscrowd': False,
                                    'tokens_positive': r.get('tokens_positive', [])
                                })
                        else:  # single box
                            x1, y1, x2, y2 = bboxes
                            w, h = x2 - x1, y2 - y1
    ...
    def parse_data_info(self, raw_data_info: dict) -> Union[dict, List[dict]]:
        ...
        for i, ann in enumerate(ann_info):
            ...
            cat_name = ' '.join([info['phrase'] for info in ann['phrase']])
