...
host = "https://9e27-35-187-228-30.ngrok-free.app"
...
def classify_category(pred_instances, caption, image_path, texts):
    # template = (
    ...
    prompt = (
        f"<|begin_of_text|><|image|>You are an AI visual assistant that can analyze a single image. You receive one image and list of categories from user input. Each category is separated by a comma. "
        f"After running the model, the result is list of detected objects, each with an index, a bounding box (x1, y1, x2, y2) where (x1, y1) is top-left corner and (x2, y2) is bottom-right corner, a label, and a confidence score.\n"
    )
    prompt += "\nDetected objects:\n"
    for i, det in enumerate(pred_instances):
        label = det["labels"][0]
        bbox = det["bboxes"][0]
        score = det["scores"][0]
    ...
    prompt += (
        f"The caption of the scene is '{caption}'\n"
        f"Based on object position and appearance, you could determine if its label is True or False\n"
        f"Please return the list index of True with format [0, 1, 2, 3, ... ], don't return any additional text.\n"
        f"Please follow the rule strictly!"
    )
...
def inference_detector(model,
    ...
    # role = classify_category(pred_instances, image_caption, image_path, texts)
    # match = re.search(r"\[\d+(?:,\s*\d+)*\]", role)

    # if match:
    #     role = match.group()
    # role = ast.literal_eval(role)
    # pred_instances = pred_instances[role]
    ...
    detections = sv.Detections(xyxy=pred_instances['bboxes'],
                               class_id=np.array(pred_instances['labels']),
